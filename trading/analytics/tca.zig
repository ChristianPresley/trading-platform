...
pub const TcaEngine = struct {
    ...
    pub fn analyze(self: *TcaEngine, executions: []const Execution, benchmark: Benchmark) !TcaReport {
        ...
        var total_qty: i64 = 0;
        for (executions) |exec| {
            total_value += exec.price * exec.quantity;
            total_qty += exec.quantity;
        }

        const avg_fill = @as(f64, @floatFromInt(total_value)) / @as(f64, @floatFromInt(total_qty));
        ...
        const is_cost_bps = is_side_sign * (avg_fill - arrival) / arrival * 10000.0;
        ...
        const timing_cost_bps: f64 = if (executions.len <= 1)
            0.0
        else blk: {
        ...
        const filled_qty = total_qty;
        const attempted = @as(f64, @floatFromInt(benchmark.attempted_qty));
        const unfilled_fraction = if (benchmark.attempted_qty > 0)
            (attempted - @as(f64, @floatFromInt(filled_qty))) / attempted
        else
            0.0;
        const opportunity_cost_bps = is_side_sign * unfilled_fraction * (close - arrival) / arrival * 10000.0;

        // VWAP slippage: (avg_fill - market_vwap) / market_vwap in bps
        const vwap_slippage_bps = is_side_sign * (avg_fill - market_vwap) / market_vwap * 10000.0;
        ...
        const fill_rate: f64 = if (benchmark.attempted_qty > 0)
            @as(f64, @floatFromInt(filled_qty)) / attempted
        else
            1.0;
