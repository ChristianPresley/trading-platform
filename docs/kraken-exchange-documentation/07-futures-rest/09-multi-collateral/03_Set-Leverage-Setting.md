...
Source: [https://docs.kraken.com/api/docs/futures-api/trading/set-leverage-setting](https://docs.kraken.com/api/docs/futures-api/trading/set-leverage-setting)
...
|-----------|------|----------|-------------|
| `symbol` | string | Yes | Symbol for the leverage preference. |
| `maxLeverage` | number | No | Max leverage to set.  When present, the symbol's margin mode will be set to "isolated". When missing, the symbol's margin mode will be set to "cross".  |
...
```bash
curl -X PUT "https://futures.kraken.com/derivatives/api/v3/leveragepreferences?symbol=PF_XBTUSD" \
...
- **Operation ID:** `setLeverageSetting`
